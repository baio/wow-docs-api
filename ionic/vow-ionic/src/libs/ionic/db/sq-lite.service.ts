...
export class SqLiteService {
    ...
    initializePlugin(): Promise<boolean> {
        return new Promise((resolve) => {
            ...
            this.isService = true;
            console.log(
                '$$$ in service this.isService ' + this.isService + ' $$$'
            );
            resolve(true);
        });
    }
    ...
    async addUpgradeStatement(
        ...
        if (this.sqlite != null) {
            ...
            return Promise.reject(
                new Error(`no connection open for ${database}`)
            );
        }
    }
    /**
    ...
    async retrieveConnection(database: string): Promise<SQLiteDBConnection> {
        if (this.sqlite != null) {
            try {
                return Promise.resolve(
                    await this.sqlite.retrieveConnection(database)
                );
            } catch (err) {
                return Promise.reject(new Error(err));
            }
    ...
    async isConnection(database: string): Promise<capSQLiteResult> {
        if (this.sqlite != null) {
            try {
                return Promise.resolve(
                    await this.sqlite.isConnection(database)
                );
            } catch (err) {
                return Promise.reject(new Error(err));
            }
    ...
    async addSQLiteSuffix(folderPath?: string): Promise<void> {
        if (this.sqlite != null) {
            try {
                ...
                return Promise.resolve(
                    await this.sqlite.addSQLiteSuffix(folderPath)
                );
            } catch (err) {
                return Promise.reject(new Error(err));
            }
